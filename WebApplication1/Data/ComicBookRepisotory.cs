...
{
    public class ComicBookRepisotory
    {
        private static ComicBook[] _comicBooks = new ComicBook[]
{
    ...
    {   Id=3,
        ...
        public ComicBook GetComicBook(int id)
        {
            ...
            foreach (var comicBook in _comicBooks) // looping through each comic book here 
            {
                if (comicBook.Id == id) // check if the id are equal or not
                { 
                    comicBookToReturn = comicBook; // if id are equal comicbookreturn inistated 
                    break;
                }
            }
